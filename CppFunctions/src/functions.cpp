...
Rcpp::List bropt_and_broptfp_Rcpp(NumericVector x) {
  ...
  NumericVector obs01  = (x - xlim[0]) / (xlim[1] - xlim[0]);
  for(int m=0; m < Mgrid.length(); m++) {
    NumericVector vec = riskhist_and_fp_Rcpp(obs01,Mgrid[m]);
    J[m] = vec[0];
    J_fp[m] = vec[1];
  }
  return Rcpp::List::create(Rcpp::Named("opt")=Mgrid[which_min(J)], Rcpp::Named("opt_fp")=std::max(5.0, Mgrid[which_min(J_fp)]));
}
